...
def generate_training_sequence(length, config, idx=None):
    ...
    if not idx:
        has_head_diff = random.choice([True, False])
        while True:
            context, instruction = random.choice(common.context_products)
            if bool(instruction.value_diff) == has_head_diff:
                break
            else:
                continue
    else:
    ...
    bits = seq_out[0]
    bits[2 + instruction.value_diff + 1] = 1
...
def seq_to_inst(seq, real):
    bits = seq[0]
    values = real[0]
    head_diff_value = max(values[2], values[3], values[4])
    value_diff = 0 if values[3] == head_diff_value else (values[2] == head_diff_value and -1) or (values[4] == head_diff_value and 1) or False
    return Instruction(0, value_diff, 0, 0, None)
