...
def generate_training_sequence(length, config, idx=None):
    ...
    if not idx:
        has_interaction = random.choice([True, False])
        if has_interaction:
            context = random.choice(pool)
            instruction = common.translator.translate(context[0], State(*context[1:]))
        else:
...
def seq_to_inst(seq, real):
    ...
    interaction_value = max(values[2], values[3], values[4])
    interaction = None if values[2] == interaction_value else (values[3] == interaction_value and 'i') or (values[4] == interaction_value and 'o') or False
    return Instruction(0, 0, 0, 0, interaction)
