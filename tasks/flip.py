...
def run(ntm, seq_length, sess, print_=True):
    ...
    feed_dict.update(
        {true_output: (vec * -1 + 1) for vec, true_output in zip(seq, ntm.true_outputs)}
    )
    feed_dict.update({
...
def train(ntm, config, sess):
    ...
    for idx in xrange(config.epoch):
        ...
        input_dict = {input_: vec for vec, input_ in zip(seq, ntm.inputs)}
        output_dict = {true_output: vec * -1 + 1 for vec, true_output in zip(seq, ntm.true_outputs)}
        ...
        if idx % 100 == 0:
            ntm.save(config.checkpoint_dir, 'flip', step)
...
def generate_copy_sequence(length, bits):
    seq_in = np.zeros([length, bits + 2], dtype=np.float32)
    for idx in xrange(length):
        rand = np.random.rand(bits).round()
        seq_in[idx, 2:bits+2] = rand
    return list(seq_in)
