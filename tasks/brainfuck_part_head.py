...
def generate_training_sequence(length, config, idx=None):
    ...
    if not idx:
        ...
        while True:
            ...
            if bool(instruction.head_diff) == has_head_diff:
                break
            else:
                continue
    else:
...
def seq_to_inst(seq, real):
    ...
    head_diff = 0 if values[3] == head_diff_value else (values[2] == head_diff_value and -1) or (values[4] == head_diff_value and 1) or False
