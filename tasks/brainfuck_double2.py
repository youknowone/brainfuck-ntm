...
def preset_flags(FLAGS):
    ...
    FLAGS.controller_dim = 200
    FLAGS.max_length = 1
    FLAGS.test_max_length = 1
    FLAGS.input_dim = 8 + 1 + 3 + 1 + 2  # 13 + 2 code 8 nonzero 1 skip 3 direction 1
    FLAGS.output_dim = 3 * (3 + 3 + 3 + 1 + 3) + 2  # 13 + 2 head 3 value 3 skip 3 direction 1 interaction 3
...
def generate_training_sequence(length, config, idx=None):
    ...
    seq_out[0][idx + subidx:idx + subidx + 3] = 1
    idx += 9
    subidx = (instruction.value_diff + 1) * 3
    seq_out[0][idx + subidx:idx + subidx + 3] = 1
    idx += 9
    subidx = (instruction.skip_diff + 1) * 3
    seq_out[0][idx + subidx:idx + subidx + 3] = 1
    idx += 9
    seq_out[0][idx:idx + 3] = direction_id
    idx += 3
    subidx = (interaction_id * 3)
    seq_out[0][idx + subidx:idx + subidx + 3] = 1
...
def seq_to_inst(seq, outputs):
    ...
    value_diff = get_diff(11)
    skip_diff = get_diff(20)
    direction = 1 if bits[29:32].sum() > 1.5 else -1
    interaction_id = get_diff(32)
    interaction = (interaction_id == 0 and 'i') or (interaction_id == 1 and 'o') or None
